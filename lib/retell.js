...
// Maps a free-text preferred language ("Spanish") to a Retell language code.
// Falls back to "multi" (multilingual) when empty or unrecognized.
...
function retellLanguageCode(text) {
  const raw = (text || "").trim();
  if (!raw) return "multi";
  if (/^[a-z]{2}(-[a-z0-9]+)?$/i.test(raw)) return raw; // already a locale code (e.g. en-US)
  return LANGUAGE_CODES[raw.toLowerCase()] || "multi";
}

// Sets the chat agent's language to match the landlord's preferred language.
export async function updateAgentLanguage(agentId, preferredLanguage) {
  const headers = authHeaders();
  if (!headers) throw new Error("RETELL_API_KEY is not set");
  await patchJSON(`/update-chat-agent/${agentId}`, headers, {
    language: retellLanguageCode(preferredLanguage),
  });
}
