...
export async function createRequest(userId, propertyId, { title, description, urgency }, { landlordEmail } = {}) {
  ...
  // Best-effort: push to the landlord's registered devices — alongside the
  // email, never instead of it. A push failure never fails the request.
  try {
    await sendPushToUser(userId, {
      title: "New maintenance request",
      body: `${property.name}: ${cleanTitle}`,
      data: { url: `/property/${propertyId}` },
    });
  } catch (e) {
    console.error("maintenance push failed:", e.message);
  }
